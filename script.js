...
async function seedDatabaseIfEmpty() {
  ...
  try {
    ...
    if (count === 0) {
      ...
      const { error: insertError } = await supabaseClient.from('resources').insert(seedItems);
  ...
  if (!supabaseClient) {
    if (dbResources.length === 0) {
      dbResources = SAMPLE_RESOURCES.map(r => ({
        id: r.id, user_id: 'mock-user-id', type: r.type, emoji: r.emoji,
        direction: r.direction, title: r.title,
        description: `Mock coordinates set in ${r.area || r.loc}.`,
        loc: r.area || r.loc, area: r.area || r.loc, km: r.km || 1.5,
        urgent: r.urgent, contact: r.contact, name: r.name,
        posted: r.posted || '5 min ago', lat: r.lat, lng: r.lng
      }));
    }
...
async function loadProfileCounts() {
  ...
  try {
    const { count: postsCount } = await supabaseClient
      .from('resources').select('*', { count: 'exact', head: true }).eq('user_id', currentUser.id);
...
window.submitLogin = async function () {
  ...
  if (!emailVal) { showToast('⚠️ Please enter your email address'); return; }
  ...
  if (!supabaseClient) {
    ...
    currentUser = { id: 'demo-user-id', email: emailVal };
    userProfile = { full_name: nameVal, profile_photo: '', current_mode: 'need_help' };
    appMode = 'seeker';
    updateTopbarAvatar();
...
window.submitGoogleLogin = async function () {
  if (!supabaseClient) {
    ...
    currentUser = { id: 'demo-google-user', email: 'demo.google@example.com' };
    userProfile = { full_name: 'Demo Google User', profile_photo: '', current_mode: 'need_help' };
    appMode = 'seeker';
    updateTopbarAvatar();
    showScreen('s-role-select');
    return;
...
window.submitGuestLogin = function () {
  showToast('🔑 Logging in to Demo Guest Account...');
  currentUser = { id: 'demo-user-id', email: 'guest@example.com' };
  userProfile = { full_name: 'Demo Guest User', profile_photo: '', current_mode: 'need_help' };
  appMode = 'seeker';
  updateTopbarAvatar();
  showScreen('s-role-select');
};
...
window.selectRole = async function (mode) {
  ...
  if (currentUser && supabaseClient) {
    ...
    await supabaseClient.from('profiles').update({ current_mode: dbMode }).eq('id', currentUser.id);
...
// ── Bottom Nav ────────────────────────────────────────────────────────────
window.appNav = async function appNav(tab) {
  if (currentTab === tab && tab !== 'home') { tab = 'home'; }
  currentTab = tab;
  document.querySelectorAll('.nb').forEach(b => b.classList.remove('on'));
  const btn = document.getElementById('nav-' + tab);
  if (btn) btn.classList.add('on');
  if (tab === 'home') { await loadResources(); renderHome(); }
  else if (tab === 'post') { renderPost(); }
  else if (tab === 'alerts') { await loadNotifications(); renderAlerts(); }
  else if (tab === 'chats') { renderChatsInbox(); }
  else if (tab === 'profile') { await loadProfileCounts(); renderProfile(); }
};
...
// ── Modal ─────────────────────────────────────────────────────────────────
window.showModal = function showModal(html) {
...
// ── Emergency SOS ─────────────────────────────────────────────────────────
window.emergencySOS = function emergencySOS() {
  showModal(`
...
function initFeedMap(resources) {
  ...
  if (!container) return;
  if (feedMap) { try { feedMap.remove(); } catch (e) { console.error(e); } feedMap = null; }
  let centerLat = 23.2599, centerLng = 77.4126;
  if (homeSearchLoc.trim()) {
    ...
    centerLat = resources[0].lat; centerLng = resources[0].lng;
  }
  feedMap = L.map('map-feed', { zoomControl: false }).setView([centerLat, centerLng], 12);
...
function renderHome() {
  ...
  const pillsHTML = CATEGORIES.map(c => `
    <div class="cat-pill ${homeCat === c.id ? 'active' : ''}" data-cat="${c.id}" onclick="homeSetCat('${c.id}')">
      ${c.emoji} ${c.label}
    </div>`).join('');
  ...
  content.innerHTML = `
    <div style="padding:14px 16px 20px;">
      <div class="live-ticker">
        ...
        <div class="ticker-scroll"><div class="ticker-inner">${tickerHTML}</div></div>
      </div>
      <div class="stat-row">
        <div class="stat-card"><div class="stat-num" style="color:#DC2626;">${appMode === 'seeker' ? '12' : '15'}</div><div class="stat-label">${appMode === 'seeker' ? 'Helpers' : 'Seekers'}</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#16A34A;">30</div><div class="stat-label">${appMode === 'seeker' ? 'Active Offers' : 'Active Requests'}</div></div>
        <div class="stat-card"><div class="stat-num" style="color:#EA580C;">5 km</div><div class="stat-label">Radius</div></div>
      </div>
      <div class="search-row">
        <div class="search-input-wrap">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input id="loc-search-input" type="text" placeholder="Search by city / area…" value="${homeSearchLoc}" onkeydown="if(event.key==='Enter') homeDoSearch()"/>
        </div>
    ...
    header = `
      <div class="loc-header">
        <button class="back-btn" onclick="homeClearSearch()">
          <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
        <h3>Results near</h3>
...
window.contactHelper = function (id, contact, name, km, emoji) {
  showModal(`
    ...
    <a href="tel:${contact}" style="display:block;text-decoration:none;margin-bottom:10px;">
      <button style="width:100%;height:50px;background:linear-gradient(135deg,#DC2626,#B91C1C);color:white;font-size:15px;font-weight:800;border:none;border-radius:14px;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 16px rgba(220,38,38,0.3);">📞 Call Now</button>
    </a>
    ...
    <button onclick="closeModal()" style="width:100%;height:46px;background:#F1F5F9;color:#475569;font-size:14px;font-weight:700;border:none;border-radius:12px;">Close</button>
  `);
};
...
window.submitPost = async function () {
  ...
  if (!type || !direction || !title || !loc || !contact) { showToast('⚠️ Please fill in all fields'); return; }
  if (!currentUser) { showToast('⚠️ Authenticated profile required'); return; }
  if (!supabaseClient) {
    showToast('Publishing resource (Demo)...');
    const center = getCoordinatesForSearch(loc);
    const newId = dbResources.length + 1;
    dbResources.unshift({
      id: newId, user_id: currentUser.id, type,
      ...
      loc, area: loc, km: 1.0, urgent, contact,
  ...
  try {
    const { data, error } = await supabaseClient.from('resources').insert([{
      user_id: currentUser.id, category: dbCategory, resource_type: direction,
      title, description: `Coordination details for ${title} listed in ${loc}.`,
      location: loc,
      latitude: center.lat + (Math.random() - 0.5) * 0.012,
      longitude: center.lng + (Math.random() - 0.5) * 0.012,
      contact_number: contact, urgency_level: urgent ? 'urgent' : 'standard', status: 'active'
    }]).select();
    if (error) throw error;
    showToast('✅ Posted successfully!');
...
window.markAlertRead = async function (id) {
  if (!supabaseClient) { showToast('Marked as read (Demo)'); return; }
  try {
    ...
    <div style="padding:16px;">
      <div style="background:#FFFFFF;border:1.5px solid #E2E8F0;border-radius:18px;padding:20px;margin-bottom:14px;text-align:center;">
        ...
        <div style="display:flex;gap:16px;justify-content:center;margin-top:14px;">
          <div style="text-align:center;"><div style="font-size:20px;font-weight:900;color:#DC2626;">${myResponseCount}</div><div style="font-size:11px;color:#94A3B8;">Chats</div></div>
          <div style="width:1px;background:#E2E8F0;"></div>
          <div style="text-align:center;"><div style="font-size:20px;font-weight:900;color:#16A34A;">${myPostCount}</div><div style="font-size:11px;color:#94A3B8;">Posts</div></div>
          <div style="width:1px;background:#E2E8F0;"></div>
          <div style="text-align:center;"><div style="font-size:20px;font-weight:900;color:#F59E0B;">⭐ 5.0</div><div style="font-size:11px;color:#94A3B8;">Rating</div></div>
        </div>
      </div>
      ${roleToggleHtml}
      ${optionsHtml}
...
window.changeAvatar = function () { document.getElementById('profile-photo-upload')?.click(); };
...
window.handleAvatarUpload = async function (file) {
  ...
  try {
    ...
    const { error: uploadError } = await supabaseClient.storage.from('avatars').upload(filePath, file, { cacheControl: '3600', upsert: true });
    if (uploadError) throw uploadError;
    const { data: urlData } = supabaseClient.storage.from('avatars').getPublicUrl(filePath);
    const publicUrl = urlData.publicUrl;
    const { error: dbError } = await supabaseClient.from('profiles').update({ profile_photo: publicUrl }).eq('id', currentUser.id);
...
window.showMyPosts = async function () {
  ...
  if (!supabaseClient) {
    const mappedPosts = dbResources.filter(r => r.user_id === currentUser.id).map(p => ({
      id: p.id, emoji: p.emoji, title: p.title, area: p.area || p.loc,
      time: p.posted, status: (p.status || 'active').charAt(0).toUpperCase() + (p.status || 'active').slice(1)
    }));
    renderMyPostsList(mappedPosts);
    return;
  }
  try {
    const { data, error } = await supabaseClient.from('resources').select('*').eq('user_id', currentUser.id).order('created_at', { ascending: false });
    if (error) throw error;
    const mappedPosts = data.map(p => ({
      id: p.id,
      emoji: p.category === 'Blood' ? '🩸' : p.category === 'Transport' ? '🚑' : p.category === 'Medicine' ? '💊' : p.category === 'Food' ? '🍱' : '🏠',
      title: p.title, area: p.location, time: timeAgo(new Date(p.created_at)),
      status: p.status.charAt(0).toUpperCase() + p.status.slice(1)
    }));
    renderMyPostsList(mappedPosts);
  } catch (err) { console.error(err); showToast('⚠️ Failed to load posts'); }
};
...
function renderMyPostsList(posts) {
  ...
  showModal(`
    ...
    <div style="max-height:300px;overflow-y:auto;padding-right:4px;">${listHtml}</div>
...
window.resolvePost = async function (postId) {
  ...
  if (!supabaseClient) {
    ...
    if (badge) { badge.textContent = 'Fulfilled'; badge.style.background = '#E2E8F0'; badge.style.color = '#64748B'; }
  ...
  try {
    const { error } = await supabaseClient.from('resources').update({ status: 'fulfilled' }).eq('id', postId);
    if (error) throw error;
    showToast('Post marked as resolved!');
    const badge = document.getElementById(`postbadge-${postId}`);
    if (badge) { badge.textContent = 'Fulfilled'; badge.style.background = '#E2E8F0'; badge.style.color = '#64748B'; }
...
window.showMyResponses = async function () {
  ...
  if (!supabaseClient) {
    ...
    showModal(`
      ...
      <div style="max-height:300px;overflow-y:auto;padding-right:4px;">${listHtml || '<div style="text-align:center;padding:20px;color:#94A3B8;">No active chats found.</div>'}</div>
      <button class="btn-secondary" onclick="closeModal()" style="margin-top:14px;">Close</button>
    `);
    return;
  }
  try {
    ...
    const listHtml = data.map(c => {
      ...
      return `
        <div style="display:flex;align-items:flex-start;gap:12px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:12px;margin-bottom:10px;">
          ...
          <div style="flex:1;min-width:0;">
            <div style="font-size:13.5px;font-weight:700;color:#0F172A;">${c.resource?.title || 'Direct Chat'}</div>
    ...
    showModal(`
      ...
      <button class="btn-secondary" onclick="closeModal()" style="margin-top:14px;">Close</button>
    `);
  } catch (err) { console.error(err); showToast('⚠️ Error loading responses'); }
};
  ...
  showModal(`
    ...
    <div style="display:flex;flex-direction:column;gap:14px;margin-bottom:20px;">
      <div style="display:flex;align-items:center;justify-content:space-between;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:12px 14px;">
        <div><div style="font-size:13.5px;font-weight:700;color:#0F172A;">Push Notifications</div><div style="font-size:11px;color:#64748B;">Get real-time request alerts</div></div>
        <div style="width:38px;height:22px;background:#16A34A;border-radius:999px;position:relative;padding:2px;cursor:pointer;" onclick="togglePref(this)"><div class="dot" style="width:18px;height:18px;background:white;border-radius:50%;position:absolute;right:2px;transition:0.2s;"></div></div>
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:12px 14px;">
        <div><div style="font-size:13.5px;font-weight:700;color:#0F172A;">Proximity SOS Alerts</div><div style="font-size:11px;color:#64748B;">Notify for SOS within 5 km</div></div>
        <div style="width:38px;height:22px;background:#16A34A;border-radius:999px;position:relative;padding:2px;cursor:pointer;" onclick="togglePref(this)"><div class="dot" style="width:18px;height:18px;background:white;border-radius:50%;position:absolute;right:2px;transition:0.2s;"></div></div>
      </div>
      <div style="display:flex;align-items:center;justify-content:space-between;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:12px 14px;">
        <div><div style="font-size:13.5px;font-weight:700;color:#0F172A;">GPS Location Access</div><div style="font-size:11px;color:#64748B;">For accurate proximity mapping</div></div>
        <div style="width:38px;height:22px;background:#16A34A;border-radius:999px;position:relative;padding:2px;cursor:pointer;" onclick="togglePref(this)"><div class="dot" style="width:18px;height:18px;background:white;border-radius:50%;position:absolute;right:2px;transition:0.2s;"></div></div>
      </div>
    </div>
...
window.togglePref = function (el) {
  ...
  if (el.style.background.includes('rgb(22, 163, 74)') || el.style.background.includes('#16A34A')) {
    ...
    if (dot) { dot.style.left = 'auto'; dot.style.right = '2px'; }
  }
};
window.saveSettings = function () { showToast('Settings saved successfully!'); closeModal(); };
window.showNotificationPrefs = function () { showToast('Preferences updated!'); };
...
window.toggleFaq = function (idx) {
  ...
  if (!ans || !icon) return;
  if (ans.style.display === 'none') { ans.style.display = 'block'; icon.textContent = '➖'; }
  else { ans.style.display = 'none'; icon.textContent = '➕'; }
};
...
window.openChat = async function (resourceId) {
  ...
  if (!resource && supabaseClient) {
    try {
      ...
      if (data) {
        resource = {
          id: data.id, user_id: data.user_id, type: data.category.toLowerCase(),
  ...
  if (!resource) {
    ...
    resource = sample ? {
      ...
      id: resourceId, user_id: 'mock-user-id', type: 'general', emoji: '💬',
  ...
  if (!supabaseClient) {
    ...
    if (!mockChats.some(c => String(c.resource_id) === String(resourceId))) {
      mockChats.push({
        id: activeChatRoomId, resource_id: resourceId, peer_name: resource.name,
        resource_title: resource.title, resource_category: resource.type, emoji: resource.emoji,
        messages: [{ sender: 'received', text: `Hello! I saw your post regarding "${resource.title}". How can I help?`, time: '10:00 AM', read: false }]
      });
    }
    ...
    <div class="chat-container">
      <div class="chat-header">
        <button class="chat-back-btn" onclick="closeChat()">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#475569" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="15 18 9 12 15 6"/></svg>
        </button>
...
window.sendChatMessage = async function () {
  ...
  if (!supabaseClient) {
    ...
    if (chatRoom) {
      chatRoom.messages.push({ sender: 'sent', text, time: getFormattedTime(new Date()), read: true });
      activeChatMessages = chatRoom.messages;
      const list = document.getElementById('chat-messages-list');
      if (list) { list.innerHTML = renderChatMessagesHTML(); scrollToChatBottom(); }
      setTimeout(() => {
        chatRoom.messages.push({ sender: 'received', text: 'This is a demo mode auto-reply. Real-time messages require a database connection.', time: getFormattedTime(new Date()), read: false });
        activeChatMessages = chatRoom.messages;
        if (list) { list.innerHTML = renderChatMessagesHTML(); scrollToChatBottom(); }
      }, 1500);
    }
  ...
  try {
    const { error } = await supabaseClient.from('messages').insert([{
      chat_id: activeChatRoomId, sender_id: currentUser.id, content: text
    }]);
    if (error) throw error;
  } catch (err) { console.error(err); showToast('⚠️ Send failed'); }
};
  ...
  activeChatSubscription = supabaseClient
    ...
    .on('postgres_changes', {
      ...
      activeChatMessages.push({
        ...
        text: msg.content, time: getFormattedTime(new Date(msg.created_at))
      });
      const list = document.getElementById('chat-messages-list');
      if (list) { list.innerHTML = renderChatMessagesHTML(); scrollToChatBottom(); }
      if (msg.sender_id !== currentUser.id) { markMessageAsRead(msg.id); }
    })
    .subscribe();
}
...
async function refreshChatBadge() {
  ...
  try {
    ...
    const { count: unreadCount } = await supabaseClient.from('messages')
      .select('*', { count: 'exact', head: true })
      .in('chat_id', chatIds).neq('sender_id', currentUser.id).eq('is_read', false);
    pendingChatUnreadCount = unreadCount || 0;
    updateChatBadge(pendingChatUnreadCount);
  } catch (err) { console.error('Chat badge refresh error:', err); }
}
...
function startGlobalSubscriptions() {
  ...
  if (globalMessageSubscription) supabaseClient.removeChannel(globalMessageSubscription);
  ...
  // ✨ Global incoming message listener — notifies the recipient instantly
  globalMessageSubscription = supabaseClient
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async payload => {
      ...
      const { data: chatData } = await supabaseClient.from('chats').select('id')
        .eq('id', msg.chat_id)
        .or(`participant_1.eq.${currentUser.id},participant_2.eq.${currentUser.id}`)
        .maybeSingle();
      if (!chatData) return;

      showToast('💬 New message received in a chat');

      if (activeChatRoomId !== msg.chat_id) {
        pendingChatUnreadCount++;
        updateChatBadge(pendingChatUnreadCount);
      }

      if (currentTab === 'chats' && currentScreen === 's-app' && !activeChatRoomId) {
        renderChatsInbox();
      }
    }).subscribe();
...
async function renderChatsInbox() {
  ...
  // Clear badge when user opens chats
  pendingChatUnreadCount = 0;
  updateChatBadge(0);

  content.innerHTML = `
    <div style="padding:16px;">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
        <div class="section-label" style="margin-bottom:0;">Active Conversations</div>
      </div>
      <div id="chats-list-container" style="display:flex;flex-direction:column;gap:10px;">
  ...
  if (!currentUser) {
    document.getElementById('chats-list-container').innerHTML = `
      <div style="text-align:center;padding:48px 20px;color:#94A3B8;">
        ...
        <div style="font-size:15px;font-weight:700;color:#475569;margin-top:10px;">Sign in required</div>
        <div style="font-size:13px;margin-top:4px;">Please log in to view your inbox.</div>
      </div>`;
    return;
  }
  ...
  if (!supabaseClient) {
    chatsData = mockChats.map(c => {
      ...
      const unreadCount = msgs.filter(m => m.sender === 'received' && !m.read).length;
      return {
        id: c.id, resource_id: c.resource_id, peer_name: c.peer_name || 'Unknown',
        title: c.resource_title || 'Direct Chat', emoji: c.emoji || '💬',
        unread_count: unreadCount,
        last_message: lastMsg ? lastMsg.text : 'No messages yet',
        last_message_time: lastMsg ? lastMsg.time : '',
        last_message_mine: lastMsg ? lastMsg.sender === 'sent' : false
      };
    });
  } else {
    try {
      ...
      chatsData = await Promise.all(data.map(async c => {
        ...
        const { count: unreadCount } = await supabaseClient.from('messages')
          .select('*', { count: 'exact', head: true }).eq('chat_id', c.id)
          .neq('sender_id', currentUser.id).eq('is_read', false);

        const { data: lastMsgArr } = await supabaseClient.from('messages')
          .select('content, sender_id, created_at').eq('chat_id', c.id)
          .order('created_at', { ascending: false }).limit(1);

        const lastMsg = lastMsgArr?.[0] || null;
        return {
          id: c.id, resource_id: c.resource_id, peer_name: peerName || 'Anonymous',
          title: c.resource?.title || 'Direct Chat', emoji,
          unread_count: unreadCount || 0,
          last_message: lastMsg ? lastMsg.content : 'No messages yet',
          last_message_time: lastMsg ? timeAgo(new Date(lastMsg.created_at)) : '',
          last_message_mine: lastMsg ? (lastMsg.sender_id === currentUser.id) : false
        };
      }));
    } catch (err) {
      console.error(err);
  ...
  if (chatsData.length === 0) {
    container.innerHTML = `
      <div style="text-align:center;padding:56px 20px;color:#94A3B8;">
        ...
        <div style="font-size:16px;font-weight:700;color:#475569;">No conversations yet</div>
        <div style="font-size:13px;margin-top:6px;line-height:1.6;max-width:240px;margin-left:auto;margin-right:auto;">
          Tap <b>Contact</b> or <b>Chat Now</b> on any resource listing to start a conversation.
        </div>
        <button onclick="appNav('home')" style="margin-top:20px;padding:10px 24px;background:linear-gradient(135deg,#DC2626,#B91C1C);color:white;border:none;border-radius:12px;font-size:13px;font-weight:700;cursor:pointer;">Browse Resources</button>
      </div>`;
    return;
  }

  container.innerHTML = chatsData.map(c => {
    const previewRaw = c.last_message_mine ? `✓ ${c.last_message}` : c.last_message;
    ...
    <div onclick="openChat('${c.resource_id}')" style="display:flex;align-items:center;gap:12px;background:#FFFFFF;border:1.5px solid ${hasUnread ? '#FCA5A5' : '#E2E8F0'};border-radius:16px;padding:14px;box-shadow:${hasUnread ? '0 2px 14px rgba(220,38,38,0.1)' : '0 1px 6px rgba(0,0,0,0.03)'};cursor:pointer;transition:all 0.15s;">
      ...
      <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:3px;">
          <span style="font-size:14.5px;font-weight:${hasUnread ? '800' : '700'};color:#0F172A;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:180px;">${c.peer_name}</span>
          <span style="font-size:11px;color:${hasUnread ? '#DC2626' : '#94A3B8'};font-weight:${hasUnread ? '700' : '400'};white-space:nowrap;flex-shrink:0;margin-left:8px;">${c.last_message_time}</span>
        </div>
        <div style="font-size:12.5px;color:${hasUnread ? '#0F172A' : '#64748B'};font-weight:${hasUnread ? '600' : '400'};white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:2px;">${preview || 'No messages yet'}</div>
        <div style="font-size:11px;color:#94A3B8;display:flex;align-items:center;gap:4px;"><span style="font-size:12px;">${c.emoji}</span> ${c.title}</div>
      </div>
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="${hasUnread ? '#DC2626' : '#CBD5E1'}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="flex-shrink:0;margin-left:4px;"><polyline points="9 18 15 12 9 6"/></svg>
    </div>`;
  }).join('');
}
